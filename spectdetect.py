...
obspy_version = obspy.core.util.version.read_release_version()
print "obspy version:", obspy_version
import obspy_spectrogram # Copied code from obspy.imaging.spectrogram source code
...
def whitened_spectrogram(st, statName, matplotlib_vs_obspy_plotting_switch):
    ...
    if data_processing_switch:
        ...
        # Or plot using obspy:
        elif matplotlib_vs_obspy_plotting_switch == 2:
            plt.figure(figsize=(24, 12))
            ax1 = plt.subplot2grid((2, 2), (0, 0), colspan=2)
            ax2 = plt.subplot2grid((2, 2), (1, 0), colspan=2)
            obspy_spectrogram.spectrogram(data,500,per_lap=0.5,wlen=1,dbscale=True,axes=ax1) # Copied code from obspy.imaging.spectrogram source code
            obspy_spectrogram.spectrogram(white_data,500,per_lap=0.5,wlen=1,dbscale=True,axes=ax2) # Copied code from obspy.imaging.spectrogram source code
...
def get_dispersion_curve_from_FTAN_array(S_t_domain_array, centre_freqs_array, time_array, plotSwitch=True):
...
def FTAN_filter_for_dispersion(centre_freq_range, centre_freq_range_step, band_width_gau_filter, sampling_rate, statName_list, fract_stations_filter, plotSwitch):
...
def produce_CMM_event_output_files(st, detection_all_stations_bins, fract_stations_filter, outdir):
...
def cut_mseed_for_events(st, detection_all_stations_bins, fract_stations_filter, outdir):
...
def STA_LTA_detect_event_arrivals_from_spectrogram_station_detection_fract(st, statName_list, detection_all_stations_bins, fract_stations_filter, outdir):
